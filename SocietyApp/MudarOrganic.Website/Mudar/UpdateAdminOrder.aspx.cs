...
public partial class Admin_UpdateOrder : System.Web.UI.Page
{
    ...
    private void BindOrderReportPathdetails()
    {
        ...
        string QorderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dtORPD = reportObj.OrderReportsPathGetDetails(Convert.ToInt32(QorderID));//Convert.ToInt32(lblBranchOrderID.Text));
    ...
    protected void btnClearance_Click(object sender, EventArgs e)
    {
        ...
        {
            orderObj.OrderDetails_UPD(Convert.ToInt32(OrderID),"RECV By<BR/>H.O", "Raghu", "<br/>" + string.Format("{0:dd MMM yyyy}", DateTime.Now) + ' ' + "UNDER CUSTOM CLEARANCE");
        }
        else
        {
        ...
        if (dtOrder.Rows.Count > 0)
        {
            ...
            else if (orderStatus == "INTRANSIT")
            {
                ...
                if (eta != "N/A")
                {
                    ...
                    btnCloseOrder.Visible = true;
                    lblETAText.Visible = true;
                    lblDispathDetails.Visible= true;
                    string Msg = Convert.ToString(dr["Comments"]);
                    if (Msg.Contains("$"))
                    {
                        string[] po = Convert.ToString(dr["Comments"]).Split('$');
                        if (po[1].ToString() != string.Empty)
                        {
                            lblDispathDetails.Text = po[1].ToString();
                            lblETAText.Text = "ETA : " + Convert.ToDateTime(eta).ToString("dd MMM yyyy");
                        }
                    }
                }
