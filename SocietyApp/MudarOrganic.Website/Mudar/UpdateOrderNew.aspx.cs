...
public partial class Admin_UpdateOrder : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            txtSelDate.Text = DateTime.Now.AddHours(13).AddMinutes(30).ToString("dd/MM/yyyy");
    ...
    private void BindddlOrderProductDetails()
    {
        ...
        ddlSelectProduct.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
        chkICSList.Visible = true;
    }
    ...
    private bool CheckQuantityStatus()
    {
        ...
        Decimal te = Convert.ToDecimal(Session["Qty"].ToString());
        return (Convert.ToDecimal(Session["Qty"].ToString()) >= qtyTotal);
    }
    ...
    {
        trOtherFarmer.Visible = false;
        txtModifyDate.Text = txtSelDate.Text;
    ...
    protected void ddlSelectProduct_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlSelectProduct.SelectedIndex > 0)
        {
            ...
            else
            {
                ...
                lblpresentqty.Text = "0";
                chkICSList.ClearSelection();
                chkVillageist.ClearSelection();
                chkVillageist.Visible = false;
                chkICSList_SelectedIndexChanged(sender, e);
                btncollectSubmit.Visible = false;
                trbtnCollectSubmit.Visible = false;
                trDate.Visible = false;
        ...
        {
            //ScriptManager.RegisterStartupScript(this, typeof(Page), "alert", "fnShowMessage('Please select Product');", true);
            chkICSList.ClearSelection();
            chkVillageist.ClearSelection();
            chkICSList.Visible = false;
            chkVillageist.Visible = false;
            trShowCollectedInfo.Visible = false;
            trShowICSList.Visible = false;
            trButtonDetails.Visible = false;
    ...
    private DataTable GetFarmerCollectionData(int productId)
    {
        ...
        {
            string[] strDate = txtSelDate.Text.Split('/');
            dtOrderCollect = orderObj.CollectedProductDetailsBasedonProductandICs(productId, Pyear.Rows[0]["ProductionYear"].ToString(), chkSelectedVal, Convert.ToDateTime(Convert.ToDateTime(strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString()).AddDays(-1)));
    ...
    private void BindgvCollecingDetails(int productID, string type)
    {
        ...
        if (type == "1")
        {
            ...
            {
                string[] strDate = txtSelDate.Text.Split('/');
                //Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
                Pyear = settObj.GetProductionYear(Convert.ToDateTime(dtDate.Rows[0]["Date"].ToString()));
                string Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
                dtGlobalFarmerCollect = orderObj.CollectedProductDetailsBasedonProductandICs(productID, Pyear.Rows[0]["ProductionYear"].ToString(), selectedVillageValue, Convert.ToDateTime(Date).AddDays(-1));
                //dtGlobalFarmerCollect = orderObj.CollectedProductDetailsBasedonProductandICs(productID, "2013", chkSelectedVal);
                decimal FarmerAqty = 0.0m;
                for (int i = 0; i < dtGlobalFarmerCollect.Rows.Count; i++)
                {
                    FarmerAqty = Convert.ToDecimal(dtGlobalFarmerCollect.Rows[i]["Avaliable"].ToString()) + FarmerAqty;

                }
                lbAqty.Text = FarmerAqty.ToString();
                DataColumn dc = new DataColumn("AvailableQuantity", typeof(decimal));
                dc.DefaultValue = 0;
                ...
                if (dtGlobalFarmerCollect.Rows.Count > 0)
                {
                    ...
                    if (ddlSelectProduct.SelectedValue == "4")
                    {
                        ...
                        if (dtmer.Rows.Count > 0)
                        {
                            ...
                            Session["Qty"] = lblOrderQuantity.Text;
                        }
                    }
                    else
                    {
        ...
        if (type == "2")
        {
            ...
            if (productID.ToString() == "4" || productID.ToString() == "10" || productID.ToString() == "11")
            {
                ...
                if (productID.ToString() == "10")
                {
                    dtOrderCollect = orderObj.PreOrderList_Product(3, chkSelectedVal);
                }
                if (productID.ToString() == "11")
                {
                    dtOrderCollect = orderObj.PreOrderList_Product(8, chkSelectedVal);
                    //productID = 8;
                }
            }
            else
    ...
    protected void btncollectSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (CheckCollectionDate())
        {
            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz check the OrderDate && enter the Vaild Date');</script>");
        }
        else
        {
            string[] strDate = txtSelDate.Text.Split('/');
            ...
            if (dtFarmercollect != null && dtPreorder != null)
            {
                ...
                for (int i = 0; i < plantIds.Length; i++)
                {
                    ...
                    {
                        PlantObj.SoldQuantity_Update(Convert.ToInt32(plantIds[i]), Convert.ToDecimal(soldQtys[i]), txtName.Text);
                    }
                }
                ...
                for (int i = 0; i < collTranscIds.Length; i++)
                {
                    ...
                    {
                        orderObj.PreorderSoldQtyUpdateNew(Convert.ToInt32(collTranscIds[i]), Convert.ToDecimal(collectedQtys[i]), txtName.Text);
                    }
                }
                ...
                if (result)
                {
                    ...
                    if (CheckPhaseCompleted(Convert.ToInt32(OrderID), Phase.Collecting))
                    {
                        ...
                        ddlSelectProduct_SelectedIndexChanged(sender, e);
                        email.EmailMethod("COLLECTION DONE", "office@mudarindia.com", OrderID + " - " + " COLLECTION DONE ");
                    }
                    else
                    {
                        ResetGrid();
                        ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('COLLECTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                        ddlSelectProduct_SelectedIndexChanged(sender, e);
                        email.EmailMethod("COLLECTION", "office@mudarindia.com", OrderID + " - " + " COLLECTION ");
                    }
                }
            }
    ...
    private void ResetGrid()
    {
        ...
        txtSelDate.Text = string.Empty;
        lbAqty.Text = string.Empty;
    ...
    #region Search option with farmer
                ...
                if (gvCollecingDetails.Rows.Count > 0)
                {
                    ...
                    .ToList().ForEach(m =>
                    {
                        ...
                        qtyTotal = qtyTotal + Convert.ToDecimal(((TextBox)item.Cells[6].FindControl("txtCollectQty")).Text);
                            ...
                            //            .ToList());
                            if (Convert.ToDecimal(((TextBox)item.Cells[6].FindControl("txtCollectQty")).Text) > Convert.ToDecimal(item.Cells[5].Text))
                            {

                                ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Collected Quantity should not exceed Farmer Available Quantity !!!');</script>");
                                ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('COLLECTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                                return;
                            }
                        }
    ...
    private void UpdateSelectedQuantityLabels()
    {
        decimal total = 0;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dtBranchOrder = orderObj.GetBranchOrderDetails(OrderID, Convert.ToInt32(ddlSelectProduct.SelectedValue));
        string Qty = dtBranchOrder.Rows[0]["NetQuantity"].ToString();
        string[] word = Qty.Split('.');
        lblOrderQuantity.Text = word[0].ToString();
        total = total + Convert.ToDecimal(hdnCollectionQuantity.Value);
        DataTable dt = (DataTable)Session["SelectedCollectionItems"];
        if (ddlSelectProduct.SelectedValue == "4")
        {
            var dtmer = set.GetMentholPerDetails(2);
            if (dtmer.Rows.Count > 0)
            {
                decimal mer = Convert.ToDecimal(dtmer.Rows[0]["percentage"].ToString());
                decimal d1 = Math.Round(((Convert.ToInt32(Qty.Split('.')[0]) * 100) / mer), 0);
                lblOrderQuantity.Text = Math.Round(d1, 2).ToString();
                Session["Qty"] = lblOrderQuantity.Text;
            }
        }
   ...
   .Select(li => li.Value).ToList();
    ...
    protected void btnBlendSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (CheckBlendDate())
        {
            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz check the collectedDate && enter the Vaild Date');</script>");
        }
        else
        {
            ...
            else
            {
                ...
                {
                    result = orderObj.ProductsBlending_Insert(ref BlendID, Convert.ToInt32(collectionID), txtBname.Text, string.Empty, MudarApp.Insert);
                }
                if (result)
                {
                    string[] strDate = txtBlendDate.Text.Split('/');
                    Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
                    result = orderObj.ProductsBlendingTran_Insert(BlendID, ProductID, FarmerId, "0", "0", "0", BlendDt, FarmId, BatchNo, lblLotNum.Text, alreadyBlendQty, txtBname.Text, Convert.ToDateTime(Date), string.Empty, MudarApp.Insert, string.Empty);
                    if (CheckBlendPhaseCompleted(Convert.ToInt32(OrderID), collectionID, Convert.ToString(BlendID)))
                    {
                        ...
                        if (dtProducts.Rows.Count > 0 && dtProducts.AsEnumerable().Any(m => m.Field<int>("ProductID") == 4))
                        {
                            ...
                            email.EmailMethod("BLENDING DONE", "office@mudarindia.com", OrderID + " - " + " BLENDING DONE ");
                        }
                        else
                        {
                            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('TESTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                    ...
                    else
                    {
                        ...
                        lblLotNum.Text = string.Empty;
                        txtBlendDate.Text = string.Empty;
                        txtBname.Text = string.Empty;
                        trBlendDate.Visible = false;
                        ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('BLENDING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                        ddlBlendProduct_SelectedIndexChanged(sender, e);
                        email.EmailMethod("BLENDING", "office@mudarindia.com", OrderID + " - " + " BLENDING ");
                        return;
                    }
                }
            }
    ...
    protected void btnGenLot_Click(object sender, EventArgs e)
    {
        ...
        if (dtDate.Rows.Count > 0)
        {
            ...
            {
                if (productID == 4 || productID == 10 || productID == 11)
                    lblLotNum.Text = APP.GenerateLotNumber(dtDate.Rows[0]["ProductCode"].ToString(), dtFinyear.Rows[0]["FYear"].ToString());
                else
                ...
                if (!string.IsNullOrEmpty(lblLotNum.Text))
                {
                    trBlendDate.Visible = true;
                    trblendSubmit.Visible = true;
                    btnBlendSubmit.Visible = true;
                }
                ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('BLENDING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
            }
        }
    ...
    private void BindddlBlendProductDetails()
    {
        ...
        ddlBlendProduct.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
    }
    ...
    private void BindBlendingDetails(string ProductID)
    {
        ...
        if (dtBlending.Rows.Count > 0)
        {
            ...
            if (dtcheck.Rows.Count > 0 && dtNewBlend.Rows.Count > 0)
            {
                ...
                dtAlreadyBlend.Columns.Add("Date", typeof(DateTime));
                foreach (DataRow item in dtNewBlend.Rows)
                {
                    ...
                    drBlendNew["Date"] = Convert.ToString(item["Date"]);
                ...
                lblBlendFarmer.Text = totalQty.ToString("F");
                if (totalQty == netQty)
                {
                    gvBlending.DataSource = null;
                    gvBlending.DataBind();
    ...
    private void BindTestResults()
    {
        ...
        DataTable dtTestingResults = orderObj.ListTestResults(collectionID, Convert.ToInt32(ddlTestProduct.SelectedValue), ddlTBlendID.SelectedItem.Text);
        ...
        else
        {
            ...
            BindTestingDetails(ddlTestProduct.SelectedValue, ddlTBlendID.SelectedItem.Text);
        }
    }
    private void BindTestingDetails(string ProductID, string BatchID)
    {
        ...
        if (dtTest.Rows.Count > 0)
        {
            ...
            string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
            DataTable dtLotNumbers = orderObj.ListLotNumbers(Convert.ToInt32(OrderID), Convert.ToInt32(ProductID), BatchID);
    ...
    protected void btnTestingSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (CheckTestedDate())
        {
            if (ddlTestProduct.SelectedValue == "4")
                ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz check the Freeze EndDate && enter the Vaild Date');</script>");
            else
                ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz check the Blending Date && enter the Vaild Date');</script>");
        ...
        else
        {
            ...
            if (result == true)
            {
                string[] strDate = txtTestDate.Text.Split('/');
                Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
                string lotNumber = gvTesting.Rows[0].Cells[0].Text;
                TextBox txtpara = (TextBox)gvTesting.Rows[0].Cells[0].FindControl("txtPara");
                TextBox txtAValue = (TextBox)gvTesting.Rows[0].Cells[0].FindControl("txtAValue");
                TextBox txtLow = (TextBox)gvTesting.Rows[0].Cells[0].FindControl("txtLow");
                TextBox txtHigh = (TextBox)gvTesting.Rows[0].Cells[0].FindControl("txtHigh");
                TextBox txtTmethod = (TextBox)gvTesting.Rows[0].Cells[0].FindControl("txtTmethod");
                orderObj.TestingResultsTrans_INT_UPT_DEL(TestID, Convert.ToInt32(ddlTestProduct.SelectedValue), lotNumber, txtpara.Text, txtAValue.Text, txtLow.Text, txtHigh.Text, txtTmethod.Text, "Bhanu", Convert.ToDateTime(Date), "Bhanu", MudarApp.Insert);
                divTestResults.Visible = false;
                ...
                if (CheckTestingCompleted())
                {
                    ...
                    else
                    {
                        ...
                        email.EmailMethod("TESTING DONE", "office@mudarindia.com", OrderID + " - " + " TESTING DONE ");
                    }
                }
                else
                {
                    BindTestResults();
                    ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('TESTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                    email.EmailMethod("TESTING", "office@mudarindia.com", OrderID + " - " + " TESTING ");
                }
            }
            else
            {
    ...
    private bool CheckCompleteTestProducts()
    {
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        int branchOrderID = Convert.ToInt32(lblBranchOrderID.Text);
        int collectionID = 0;
        bool testingCompleted = false;
        DataTable dtTestingResults = orderObj.OrderProductList(Convert.ToInt32(OrderID), branchOrderID);
        for (int i = 0; i < dtTestingResults.Rows.Count; i++)
        {
            DataTable dtcollec = orderObj.GetCollectionID(OrderID);
            if (dtcollec.Rows.Count > 0)
            {
            ...
            DataTable dtTestResults = orderObj.ListTestResults(collectionID, Convert.ToInt32(ddlTestProduct.SelectedValue), ddlTBlendID.SelectedItem.Text);
    ...
    private void BindddlTestProductDetails()
    {
        ...
        ddlTestProduct.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
    }
    private bool CheckTestingCompleted()
    {
        ...
        foreach (DataRow item in dtProducts.Rows)
        {
            DataTable dtBlendID = orderObj.GetBlendIDS(OrderID, item["ProductID"].ToString());
            foreach (DataRow dr in dtBlendID.Rows)
            {
                DataTable dt1 = orderObj.CheckTestingCompleted(dr["Blending_BatchID"].ToString(), item["ProductID"].ToString());
                if (dt1.Rows.Count <= 0)
                {
    ...
    private bool checkPackingCompleted()
    {
        bool PackingCompleted = true;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dtProducts = orderObj.ListOrderProducts(Convert.ToInt32(OrderID));
        foreach (DataRow item in dtProducts.Rows)
        {
            ...
            foreach (DataRow dr in dtBlendID.Rows)
            {
                DataTable dt1 = orderObj.CheckPackingCompleted(dr["Blending_BatchID"].ToString(), item["ProductID"].ToString());
                if (dt1.Rows.Count <= 0)
                {
    ...
    private void BindBlendID()
    {
        divTestingCompleted.Visible = false;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dt = orderObj.GetBlendIDS(OrderID, ddlTestProduct.SelectedValue);
        if (dt.Rows.Count > 0)
        {
            ddlTBlendID.DataSource = dt;
            ddlTBlendID.DataTextField = "Blending_BatchID";
            ddlTBlendID.DataValueField = "Blending_BatchID";
            ddlTBlendID.DataBind();
            ddlTBlendID.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
        }
        ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('TESTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
    }

    protected void ddlTBlendID_SelectedIndexChanged(object sender, EventArgs e)
    {
        string QorderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        ...
        {
            BindTestResults();
            if (lblBOtype.Text == "LotSample")
            {
                if (CheckCompleteTestProducts())
                {
                    txtQty.Text = string.Empty;
                    txtLSmsg.Text = string.Empty;
                    string check = string.Empty;
                    dtgetDetails = orderObj.GetSampleQtyandMsg(ddlTBlendID.Text, ddlTestProduct.SelectedValue);
                    if (dtgetDetails.Rows.Count > 0)
                    {
                        txtQty.Text = dtgetDetails.Rows[0][1].ToString();
                        txtLSmsg.Text = dtgetDetails.Rows[0][2].ToString();
    ...
    {
        //BindddlBlendIDdetails();
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        ddlBlendID.DataSource = orderObj.GetBlendID(OrderID, "4");
        ddlBlendID.DataTextField = "Blending_BatchID";
        ddlBlendID.DataValueField = "BlendingTransactionID";
        ddlBlendID.DataBind();
        ddlBlendID.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
    }
    protected void ddlBlendID_SelectedIndexChanged(object sender, EventArgs e)
    {
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataSet dtFreeze = orderObj.CheckBlendIDInsert(Convert.ToInt32(ddlBlendID.SelectedValue.ToString()));
        decimal netQty = 0.0M;
        string BlendingBatchID = string.Empty;
        DataSet dsFT = new DataSet();
        DataTable dtmer = new DataTable();
        DataTable dtFreeze1 = new DataTable();
        string Pname = string.Empty;
        if (dtFreeze.Tables[0].Rows.Count > 0)
        {
            //dsFT = orderObj.GetFreezeTran(dtFreeze.Tables[0].Rows[0]["FreezeID"].ToString());
            if (dtFreeze.Tables[1].Rows.Count > 0)
            {
                dtFreeze.Tables[0].Columns.Add("Productname");
                dtFreeze.Tables[0].Columns.Add("Blending_BatchID");
                gvFreeze.DataSource = dtFreeze;
                for (int i = 0; i < dtFreeze.Tables[0].Rows.Count; i++)
                {
                    if (dtFreeze.Tables[0].Rows[i][8].ToString() == "4")
                    {
                        netQty = Convert.ToDecimal(dtFreeze.Tables[0].Rows[i]["Quntatiy"].ToString());
                        Pname = "Organic Cornmint Oil";
                        dtFreeze.Tables[0].Rows[i]["Productname"] = Pname;
                        dtFreeze.Tables[0].Rows[i]["Blending_BatchID"] = dtFreeze.Tables[2].Rows[0]["Blending_BatchID"].ToString();
                        //string[] strDate = dtFreeze.Tables[0].Rows[i]["StartDate"].ToString().Split(' ');
                        //string[] strDate2 = strDate[0].ToString().Split('/');
                        //string SDate = strDate2[1].ToString() + "/" + strDate2[0].ToString() + "/" + strDate2[2].ToString();
                        //string[] strDate1 = dtFreeze.Tables[0].Rows[i]["EndDate"].ToString().Split(' ');
                        //string[] strDate3 = strDate1[0].ToString().Split('/');
                        //string EDate = strDate3[1].ToString() + "/" + strDate3[0].ToString() + "/" + strDate3[2].ToString();
                        //dtFreeze.Tables[0].Rows[i]["StartDate"] = SDate.ToString();
                        //dtFreeze.Tables[0].Rows[i]["EndDate"] = EDate.ToString();
                    }
                }
                gvFreeze.DataBind();
                gvFreeze.Rows[0].FindControl("btnFreez").Visible = false;
                dlFreeze.DataSource = dtFreeze.Tables[0];
                dlFreeze.DataBind();
                (dlFreeze.Items[0].FindControl("gvFreezeTran") as GridView).DataSource = dtFreeze.Tables[1];
        ...
        {
            dtFreeze1 = orderObj.GetFreezeDetails(OrderID, !string.IsNullOrEmpty(lblBranchOrderID.Text) ? lblBranchOrderID.Text : "0", Convert.ToInt32(ddlBlendID.SelectedValue.ToString()));
            if (dtFreeze1.Rows.Count > 0)
            {
                dtFreeze1.Columns.Add("Quntatiy", typeof(Int32));
                dtFreeze1.Columns.Add("BlendingTransID", typeof(Int32));
                dtFreeze1.Columns.Add("Blending_BatchID", typeof(string));
                gvFreeze.DataSource = dtFreeze1;
                for (int i = 0; i < dtFreeze1.Rows.Count; i++)
                {
                    if (dtFreeze1.Rows[i][3].ToString() == "4")
                    {
                        DataTable dt = orderObj.GetBlendID(OrderID, "4", ddlBlendID.SelectedValue.ToString());
                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            if (ddlBlendID.Text == dt.Rows[j]["BlendingTransactionID"].ToString())
                            {
                                netQty = Convert.ToDecimal(dt.Rows[j]["bqty"].ToString());
                                BlendingBatchID = dt.Rows[j]["Blending_BatchID"].ToString();
                            }
                        }
                        Pname = "Organic Cornmint Oil";
                        //dtmer = set.GetMentholPerDetails(2);
                        //decimal mer = Convert.ToDecimal(dtmer.Rows[0]["percentage"].ToString());
                        //decimal d1 = Math.Round(((netQty * 100) / mer), 0);
                        //dtFreeze1.Rows[i][4] = d1.ToString();
                        dtFreeze1.Rows[i]["Blending_BatchID"] = BlendingBatchID;
                        dtFreeze1.Rows[i]["Quntatiy"] = Convert.ToInt32(netQty).ToString();
                        dtFreeze1.Rows[i][5] = Pname;
                        dtFreeze1.Rows[i][7] = dtFreeze1.Rows[i]["FBatchID"].ToString();
                    }
                }
                gvFreeze.DataBind();
                dlFreeze.Visible = false;
            }
            else
            {
                btnFreezeSubmit.Visible = false;
    ...
    private void BindddlBlendIDdetails()
    {
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        //DataSet dtFreeze = orderObj.CheckBlendIDInsert(Convert.ToInt32(ddlBlendID.SelectedValue.ToString()));
        DataSet dtFreeze = orderObj.CheckBlendIDInsert(OrderID);
        decimal netQty = 0.0M;
        DataSet dsFT = new DataSet();
        DataTable dtmer = new DataTable();
        DataTable dtFreeze1 = new DataTable();
            ...
            if (dtFreeze.Tables[1].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtFreeze.Tables[0].Rows.Count; i++)
                {
                    if (dtFreeze.Tables[0].Rows[i][8].ToString() == "4")
                    {
                        ...
                        decimal mer = Convert.ToDecimal(dtmer.Rows[0]["percentage"].ToString());
                        decimal d1 = Math.Round(((netQty * 100) / mer), 0);
                        dtFreeze.Tables[0].Rows[i][9] = d1.ToString();
                        dtFreeze.Tables[0].Rows[i]["Productname"] = Pname;
                        dtFreeze.Tables[0].Rows[i]["Blending_BatchID"] = "ddd";
                        dtFreeze.Tables[0].Rows[i][10] = dtFreeze.Tables[0].Rows[i]["FBatchID"].ToString();
                    }
                }
                gvFreeze.DataBind();
                gvFreeze.Rows[0].FindControl("btnFreez").Visible = false;
        ...
        {
            dtFreeze1 = orderObj.BindBlendTranID(OrderID);
            dtFreeze1.Columns.Add("FreezeID");
            dtFreeze1.Columns.Add("ProductName");
            dtFreeze1.Columns.Add("FBatchID");
            dtFreeze1.Columns.Add("StartDate");
            dtFreeze1.Columns.Add("EndDate");
            if (dtFreeze1.Rows.Count > 0)
            {
                gvFreeze.DataSource = dtFreeze1;
                for (int i = 0; i < dtFreeze1.Rows.Count; i++)
                {
                    if (dtFreeze1.Rows[i][3].ToString() == "4")
                    {
                        dtFreeze1.Rows[i][5] = "Organic Cornmint Oil";
                    }
                }
                gvFreeze.DataBind();
            }
    ...
    protected void gvFreeze_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        //int BID = Convert.ToInt32(dk.Values["BlendingTransID"].ToString());
        //int qty = Convert.ToInt32(dk.Values["Quntatiy"].ToString());
        //int qty = ;
            ...
            case "Freeze":
                {
                    if (fid == 0)
                    {
                        divFreezeSubmit.Visible = true;

                        string qt = gvFreeze.Rows[index].Cells[1].Text;
                        int qty = Convert.ToInt32(gvFreeze.Rows[index].Cells[1].Text);
                        if (!string.IsNullOrEmpty((gvFreeze.Rows[index].FindControl("txtStartDate") as TextBox).Text))
                        {
                            string[] SstrDate = (gvFreeze.Rows[index].FindControl("txtStartDate") as TextBox).Text.Split('/');
                            string SDate = SstrDate[1].ToString() + "/" + SstrDate[0].ToString() + "/" + SstrDate[2].ToString();
                            StartDate = Convert.ToDateTime(SDate);
                        }
                        else
                        {
                            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz enter the Start Date');</script>");
                            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('FREEZING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                            return;
                        }
                        if (!string.IsNullOrEmpty((gvFreeze.Rows[index].FindControl("txtEndDate") as TextBox).Text))
                        {
                            string[] EstrDate = (gvFreeze.Rows[index].FindControl("txtEndDate") as TextBox).Text.Split('/');
                            string EDate = EstrDate[1].ToString() + "/" + EstrDate[0].ToString() + "/" + EstrDate[2].ToString();
                            ...
                            {
                                (gvFreeze.Rows[index].FindControl("txtEndDate") as TextBox).Text = string.Empty;
                                ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('EndDate should be greater than StartDate');</script>");
                                ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('FREEZING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                                return;
                            }
                        }
                        ...
                        {
                            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz enter the End Date');</script>");
                            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('FREEZING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                            return;
                        }
                        result = orderObj.Freeze_Insert(orderid, Convert.ToInt32(lblBranchOrderID.Text), "Aslam", string.Empty, qty, pid, fBatchID, MudarApp.Insert, StartDate, EndDate, Convert.ToInt32(ddlBlendID.SelectedValue), ref fid);
                        if (result)
                        {
                        ...
                        {
                            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('something went wrong');</script>");
                            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('FREEZING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                            return;
                        }
                    }
    ...
    private void BindFreezeTransaction(string FID, string ProductID)
    {
        ...
        if (dsFT.Tables[0].Rows.Count > 0)
        {
            divFreezeSubmit.Visible = true;
            btnDisableFrzee.Visible = false;
            btnFreezeSubmit.Visible = true;
            dlFreeze.Visible = true;
            dsFT.Tables[0].Columns.Add("Productname");
            dsFT.Tables[0].Columns.Add("Blending_BatchID");
            gvFreeze.DataSource = dsFT.Tables[0];
            for (int i = 0; i < dsFT.Tables[0].Rows.Count; i++)
            {
                if (dsFT.Tables[0].Rows[i][8].ToString() == "4")
                {
                    dsFT.Tables[0].Rows[i]["Productname"] = "Organic Cornmint Oil";
                    //string[] strDate = dsFT.Tables[0].Rows[i]["StartDate"].ToString().Split(' ');
                    //string[] strDate2 = strDate[0].ToString().Split('/');
                    //string SDate = strDate2[1].ToString() + "/" + strDate2[0].ToString() + "/" + strDate2[2].ToString();
                    //string[] strDate1 = dsFT.Tables[0].Rows[i]["EndDate"].ToString().Split(' ');
                    //string[] strDate3 = strDate1[0].ToString().Split('/');
                    //string EDate = strDate3[1].ToString() + "/" + strDate3[0].ToString() + "/" + strDate3[2].ToString();
                    //dsFT.Tables[0].Rows[i]["StartDate"] = SDate;
                    //dsFT.Tables[0].Rows[i]["EndDate"] = EDate;
                }
                DataTable dt = orderObj.GetBlendID(OrderID, "4", ddlBlendID.SelectedValue.ToString());
                if (ddlBlendID.Text == dt.Rows[0]["BlendingTransactionID"].ToString())
                {
                    dsFT.Tables[0].Rows[i]["Blending_BatchID"] = dt.Rows[0]["Blending_BatchID"].ToString();
                }
            }
            gvFreeze.DataBind();
            gvFreeze.Rows[0].FindControl("btnFreez").Visible = false;
    ...
    protected void btnFreezeSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (CheckingFreezePhaseComplete())
        {
            ...
            Session["POStatus"] = "FREEZING";
            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('TESTING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
            email.EmailMethod("FREEZING DONE", "office@mudarindia.com", OrderID + " - " + " FREEZING DONE ");
        }
        else
        {
            //btnDisableFrzee.Visible = true;
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('FREEZING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
            email.EmailMethod("FREEZING", "office@mudarindia.com", OrderID + " - " + " FREEZING ");
        }
    ...
    #region Packing
    private void BindddlPackingProductDetails()
    {
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        int branchOrderID = Convert.ToInt32(lblBranchOrderID.Text);
        ddlPackingProduct.DataSource = orderObj.OrderProductList(Convert.ToInt32(OrderID), branchOrderID);
        ddlPackingProduct.DataTextField = "ProductName";
        ddlPackingProduct.DataValueField = "ProductID";
        ddlPackingProduct.DataBind();
        ddlPackingProduct.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
    }
    protected void ddlPackingProduct_SelectedIndexChanged(object sender, EventArgs e)
    {
        BindPackingBlendID();
    }
    protected void ddlPBlendID_SelectedIndexChanged(object sender, EventArgs e)
    {
        BindNewPackingDetail();
    }
    private void BindPackingBlendID()
    {
        gvPackingCompleted.Visible = false;
        btnDisablePacking.Visible = false;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dt = orderObj.GetBlendIDS(OrderID, ddlPackingProduct.SelectedValue);
        if (dt.Rows.Count > 0)
        {
            ddlPBlendID.DataSource = dt;
            ddlPBlendID.DataTextField = "Blending_BatchID";
            ddlPBlendID.DataValueField = "Blending_BatchID";
            ddlPBlendID.DataBind();
            ddlPBlendID.Items.Insert(0, MudarApp.AddListItemWithDefaultValue());
        }
        ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('PACKING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
    }
    protected void btnPackingDetailsSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (CheckPackingDate())
        {
            ClientScript.RegisterStartupScript(typeof(Page), "alert", "<script language=JavaScript>alert('Plz check the Testing Date && Enter the Vaild Date');</script>");
            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('PACKING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
        }
        else
        {
            ...
            string[] strDate = txtPackingDate.Text.Split('/');
            Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
            foreach (GridViewRow Row in gvPackingDetails.Rows)
            {
                ...
                bool packingResult = orderObj.OrderProductPackingInsert(Convert.ToInt32(collectionId), ProductID, Lotnumber, Packing25, Packing180, drum25from, drum25to, drum180from, drum180to, Convert.ToDateTime(Date));
            ...
            if (result)
            {
                ...
                if (checkPackingCompleted())
                {
                    ...
                    email.EmailMethod("PACKING DONE", "office@mudarindia.com", OrderID + " - " + " PACKING DONE ");
                }
                else
                {
                    BindNewPackingDetail();
                    ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('PACKING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
                    email.EmailMethod("PACKING", "office@mudarindia.com", OrderID + " - " + " PACKING ");
                    return;
                }
            }
            else
    ...
    private void BindNewPackingDetail()
    {
        ...
        if (dtcollec.Rows.Count > 0)
            collectionId = dtcollec.Rows[0]["CollectionID"].ToString();
        DataTable dtOrderPacking = orderObj.CheckPackingCompleted(ddlPBlendID.SelectedItem.Text, ddlPackingProduct.SelectedItem.Value);
        if (dtOrderPacking.Rows.Count > 0)
        {
            gvPackingCompleted.Visible = true;
            gvPackingCompleted.DataSource = dtOrderPacking;
            gvPackingCompleted.DataBind();
            gvPackingDetails.DataSource = null;
            gvPackingDetails.DataBind();
            btnPackingDetailsSubmit.Visible = false;
            trPackDate.Visible = false;
            btnDisablePacking.Visible = true;
            ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('PACKING','" + Convert.ToString(Session["POStatus"]) + "','" + Convert.ToString(Session["hasMenthol"]) + "','" + Convert.ToString(Session["OrderType"]) + "');", true);
        }
        else
        {
            ...
            foreach (DataRow prodItem in orderProducts.Rows)
            {
                ...
                {
                    DataTable dtFBID = orderObj.GetPackingDetailsforCrystal(ddlPBlendID.SelectedItem.Text, Convert.ToString(prodItem["ProductID"]));
                    if (dtFBID.Rows.Count > 0)
                    {
                        foreach (DataRow item in dtFBID.Rows)
                        {
                            DataRow drBlendNew = dtAlreadyBlend.NewRow();
                            drBlendNew["Blending_BatchID"] = Convert.ToString(item["FreezeProductBatchID"]);
                            drBlendNew["BlendQty"] = Convert.ToString(item["FreezeQuantity"]);
                            dtAlreadyBlend.Rows.Add(drBlendNew);
                        }
                    }
                }
                else
                {
                    ...
                    if (dtBlending.Rows.Count > 0)
                    {
                        ...
                        if (dtcheck.Rows.Count > 0)
                        {
                            dtNewBlend = orderObj.GetPackingDetails(ddlPBlendID.SelectedItem.Text, Convert.ToString(prodItem["ProductID"]));
                        }
                        if (dtcheck.Rows.Count > 0 && dtNewBlend.Rows.Count > 0)
                        {
                            foreach (DataRow item in dtNewBlend.Rows)
                ...
                foreach (DataRow item in dtAlreadyBlend.Rows)
                {
                    ...
                    decimal grossQuantity = 0.00M;
                    int pack25 = 0;
                    int pack180 = 0;
                    int check = 0;
                    //string ss1 = Convert.ToString(item["BlendQty"]) + "0";
                    //string ss = Convert.ToString(prodItem["Quantity"]) + ".00";
                    if (item["BlendQty"].ToString() + "0" == Convert.ToString(prodItem["Quantity"]) + ".00")
                    {
                    ...
                    else
                    {
                        if (Convert.ToString(prodItem["ProductID"]) == "4")
                        {
                            if (quantity != 0)
                            {
                                pack25 = Convert.ToInt32(quantity / 25);
                        ...
                        else
                        {
                            if (quantity >= 180)
                            {
                                check = Convert.ToInt32(quantity / 180);
                                if (check == 0)
                                {
    ...
    {
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        string[] strDate = txtDispatchdate.Text.Split('/');
        string Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
        orderObj.BranchOrderDetails_UPD(Convert.ToInt32(lblBranchOrderID.Text), "DISPATCH", "Raghu", "<br/>" + string.Format("{0:dd MMM yyyy}", DateTime.Now) + ' ' + "DISPATCH", Convert.ToDateTime(Date), txtInvno.Text);
        //ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('DISPATCH','" + Convert.ToString(Session["POStatus"]) + "');", true);
        email.EmailMethod("DISPATCH DONE", "office@mudarindia.com", OrderID + " - " + " DISPATCH DONE ");
        Response.Redirect("~/Mudar/BranchOrder.aspx");
    }
    ...
    protected void btnReportSubmit_Click(object sender, EventArgs e)
    {
        ...
        email.EmailMethod("DOCUMENTING DONE", "office@mudarindia.com", OrderID + " - " + " DOCUMENTING DONE ");
        //email.EmailMethod("PACKING", "office@mudarindia.com", OrderID + " - " + " PACKING ");
        //ScriptManager.RegisterStartupScript(this, GetType(), "myFunction", "ManageTabs('DISPATCH','" + Convert.ToString(Session["POStatus"]) + "');", true);
    }
    #endregion
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
        bool result;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        result = orderObj.InsertSampleQtyandMsgDetails(txtQty.Text + " " + " gm.", string.Format("{0:dd MMM yyyy}", DateTime.Now.AddHours(12).AddMinutes(30)) + " " + txtLSmsg.Text, Convert.ToInt32(OrderID), Convert.ToInt32(ddlTestProduct.SelectedValue), ddlTBlendID.SelectedItem.Text);
        if (result)
        {
            orderObj.BranchOrderDetails_UPD(Convert.ToInt32(lblBranchOrderID.Text), "SAMPLE DISPATCH", "Raghu", "SAMPLE DISPATCH");
            string Sub = txtQty.Text + " " + " gm." + string.Format("{0:dd MMM yyyy}", DateTime.Now.AddHours(12).AddMinutes(30)) + " " + txtLSmsg.Text;
            email.EmailMethod(Sub, "office@mudarindia.com", OrderID + " - " + " SAMPLE DISPATCH ");
        }
        Response.Redirect("~/Mudar/BranchOrder.aspx");
    }
    private bool CheckCollectionDate()
    {
        DataTable dtDate = new DataTable();
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        string Date = string.Empty;
        bool ret = false;
        if (!string.IsNullOrEmpty(txtModifyDate.Text))
        {
            string[] strDate = txtModifyDate.Text.Split('/');
            Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
            dtDate = orderObj.GetBranchOrderDetails(Convert.ToInt32(OrderID));
            DateTime SDate = Convert.ToDateTime(dtDate.Rows[0]["CreatedDate"]);
            DateTime FDate = Convert.ToDateTime(SDate.ToShortDateString());
            if (Convert.ToDateTime(Date) > DateTime.Now.AddHours(12).AddMinutes(30))
                //if (Convert.ToDateTime(Date) > DateTime.Now)
                ret = true;
            if (FDate > Convert.ToDateTime(Date))
                ret = true;
        }
        else
            ret = true;
    ...
    private bool CheckBlendDate()
    {
        DataTable dtDate = new DataTable();
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        string Date = string.Empty;
        bool ret = false;
        if (!string.IsNullOrEmpty(txtBlendDate.Text))
        {
            string[] strDate = txtBlendDate.Text.Split('/');
            Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
            dtDate = orderObj.GetCollectionTransDate(OrderID, ddlBlendProduct.SelectedValue);
            DateTime SDate = Convert.ToDateTime(dtDate.Rows[0]["CreatedDate"]);
            DateTime FDate = Convert.ToDateTime(SDate.ToShortDateString());
            if (Convert.ToDateTime(Date) > DateTime.Now.AddHours(12).AddMinutes(30))
                ret = true;
    ...
    private bool CheckTestedDate()
    {
        ...
        if (!string.IsNullOrEmpty(txtTestDate.Text))
        {
            string[] strDate = txtTestDate.Text.Split('/');
            Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
            dtDate = orderObj.GetBlendTransUpdatedDate(OrderID, ddlTestProduct.SelectedValue, ddlTBlendID.SelectedItem.Text);
            DateTime SDate = Convert.ToDateTime(dtDate.Rows[0]["CreatedDate"]);
            DateTime FDate = Convert.ToDateTime(SDate.ToShortDateString());
            if (Convert.ToDateTime(Date) > DateTime.Now.AddHours(12).AddMinutes(30))
                ret = true;
    ...
    private bool CheckPackingDate()
    {
        DataTable dtDate = new DataTable();
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        string Date = string.Empty;
        bool ret = false;
        if (!string.IsNullOrEmpty(txtPackingDate.Text))
        {
            string[] strDate = txtPackingDate.Text.Split('/');
            Date = strDate[1].ToString() + "/" + strDate[0].ToString() + "/" + strDate[2].ToString();
            dtDate = orderObj.GetTestTransUpdateDate(OrderID, ddlPBlendID.SelectedItem.Text);
            DateTime SDate = Convert.ToDateTime(dtDate.Rows[0]["CreatedDate"]);
            DateTime FDate = Convert.ToDateTime(SDate.ToShortDateString());
            if (Convert.ToDateTime(Date) > DateTime.Now.AddHours(12).AddMinutes(30))
                ret = true;
    ...
    private bool CheckingFreezePhaseComplete()
    {
        bool ret = false;
        string OrderID = Encrypt_Decrypt.Decrypt(Session["sOrderID"].ToString().Trim(), true);
        DataTable dt = orderObj.GetBlendID(OrderID, "4");
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            DataSet dtFreeze = orderObj.CheckBlendIDInsert(Convert.ToInt32(dt.Rows[i]["BlendingTransactionID"].ToString()));
            if (dtFreeze.Tables[0].Rows.Count > 0)
            {
                if (dtFreeze.Tables[1].Rows.Count > 0)
                    ret = true;
    ...
    protected void gvTesting_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            var lblMinAnalysisValue = e.Row.FindControl("lblMinAnalysisValue") as Label;
            var lblMaxAnalysisValue = e.Row.FindControl("lblMaxAnalysisValue") as Label;
            var randomAnalysisValue = MudarApp.RandomNumber(Convert.ToDecimal(lblMinAnalysisValue.Text), Convert.ToDecimal(lblMaxAnalysisValue.Text));
            var txtAValue = e.Row.FindControl("txtAValue") as TextBox;
            NumberFormatInfo setPrecision = new NumberFormatInfo();
            setPrecision.NumberDecimalDigits = 1;
            txtAValue.Text = randomAnalysisValue.ToString("N", setPrecision);
        }
    }
}